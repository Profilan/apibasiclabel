 ...
 */
class OrderRegelMapper implements MapperInterface
{
    /**
     * 
     * @var EntityManager
    ...
    {
        $item = $this->entityManager->find('Webshop\\V1\Rest\OrderRegel\OrderRegelEntity', $id);
        
        return $item; 
    }
    
    /**
     * @return OrderRegelCollection
     */
    public function fetchAll($params = array())
    {
        if ($params->get('REFERENTIE')) {
            $items = $this->entityManager->getRepository('Webshop\\V1\Rest\OrderRegel\OrderRegelEntity')->findBy(array(
               'referentie' => $params->get('REFERENTIE') 
            ));
        } else {
            $items = $this->entityManager->getRepository('Webshop\\V1\Rest\OrderRegel\OrderRegelEntity')->findAll();
        }
        $collection = new ArrayCollection($items);
        
        return new OrderRegelCollection(new Collection($collection));
    }
