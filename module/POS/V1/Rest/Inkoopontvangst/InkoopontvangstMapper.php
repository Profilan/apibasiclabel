...
class InkoopontvangstMapper implements MapperInterface
{
    ...
    {
        $item = $this->entityManager->find('POS\V1\Rest\Inkoopontvangst\InkoopontvangstEntity', $id);
    ...
    public function fetchAll($params = array())
    {
        if ($params->get('REFERENTIE')) {
            $items = $this->entityManager->getRepository('POS\V1\Rest\Inkoopontvangst\InkoopontvangstEntity')->findBy(array(
               'referentie' => $params->get('REFERENTIE') 
            ));
        } else {
            $items = $this->entityManager->getRepository('POS\V1\Rest\Inkoopontvangst\InkoopontvangstEntity')->findAll();
        }
