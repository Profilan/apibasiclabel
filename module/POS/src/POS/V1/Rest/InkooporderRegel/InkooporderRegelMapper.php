...
class InkooporderRegelMapper implements MapperInterface
{
    ...
    {
        $item = $this->entityManager->find('Webshop\\V1\Rest\InkooporderRegel\InkooporderRegelEntity', $id);
    ...
    public function fetchAll($params = array())
    {
        if ($params->get('REFERENTIE')) {
            $items = $this->entityManager->getRepository('Webshop\\V1\Rest\InkooporderRegel\InkooporderRegelEntity')->findBy(array(
               'referentie' => $params->get('REFERENTIE') 
            ));
        } else {
            $items = $this->entityManager->getRepository('Webshop\\V1\Rest\InkooporderRegel\InkooporderRegelEntity')->findAll();
        }
